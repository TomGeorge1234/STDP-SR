...
T_SR = [4]
T_STDP = [10e-3,20e-3,30e-3]
T_STDP_ASYMM = [1,1.5,2,2.5]
A_STDP_ASYMM = [-0.7,-0.8,-0.9,-1,-1.1]
F = [0.4,0.6,0.8,1]
K = [0.5,1,1.5,2]
FR = [1,5,20,50]
            ...
            for t_stdp_asymm in T_STDP_ASYMM:
                for a_stdp_asymm in A_STDP_ASYMM:
                    for f in F:
                        for k in K:
                            for fr in FR:
                                new.write("srun --ntasks=1 --nodes=1 python clusterSweep.py %f %f %f %f %f %f %f &" %(t_sr, t_stdp, t_stdp_asymm, a_stdp_asymm, f, k, fr))
                                new.write("\n")
    new.write("wait")
