...
for i in range(5):
    ...
    R_30_w, R_30_nw, SNR_30_w, SNR_30_nw = agent.getMetrics(time=30)

    data = [str(t_sr),str(t_stdp),str(t_stdp_asymm),str(a_stdp_asymm), str(f), str(k), str(round(R_10_w,5)), str(round(R_10_nw,5)), str(round(SNR_10_w,5)), str(round(SNR_10_nw,5)), str(round(R_30_w,5)), str(round(R_30_nw,5)), str(round(SNR_30_w,5)), str(round(SNR_30_nw,5))]
    with open("sweepResults.txt", "a") as f: 
        if sum(1 for line in open('sweepResults.txt')) == 0:
            f.write("t_sr,t_stdp,t_stdp_asymm,a_stdp_asymm,f,k,R_30_w,R_30_nw,SNR_30_w,SNR_30_nw")
            f.write("\n")
        f.write(','.join(data))
        f.write('\n')
